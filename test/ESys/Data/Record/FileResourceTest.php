...
class ESys_Data_Record_FileResourceTest extends PHPUnit_Framework_TestCase {
    ...
    public function testInstallsFile ()
    {
        ...
        $this->assertTrue(
            $fileResource->install($this->sourceFile, basename($this->sourceFile)),
    ...
    {
        $urlBase = '/subdir';
        $fileResource = new ESys_Data_Record_FileResource($this->storageDir, $urlBase);
        $fileResource->install($this->sourceFile, basename($this->sourceFile));
        $expectedUrl = $urlBase.'/'.basename($this->sourceFile);
